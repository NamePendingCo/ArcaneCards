...
## the event resolving system in the battle manager.
## 
## They should usually only exist to run once and then disappear. Rarely should this ever 
## be created directly. Instead we should rely on EventLaunchers to create these based on  
...
## Have the event go through and make all selections for the parameters
## and conditions. Then send signal that this event is running.
func prepare_to_run():
...
## the score, the earlier it goes. [br][br]
## Returns: [br]
